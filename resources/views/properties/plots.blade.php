{{-- resources/views/properties/plots.blade.php --}}
@extends('layouts.app')

@section('title', 'Земельные участки')
...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="relative rounded-2xl overflow-hidden mb-12 bg-gradient-to-r from-purple-600 to-indigo-600">
        ...
        <div class="relative z-10 py-10 px-8 text-white">
            <h1 class="text-4xl md:text-5xl font-bold mb-3">Земельные участки</h1>
            <p class="text-lg md:text-xl opacity-90">Найдите идеальный участок для строительства дома мечты</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-xl p-6 mb-8 border border-gray-100">
        <form method="GET" action="{{ route('properties.plots') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div>
                ...
                <div class="relative">
                    <select disabled class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl bg-gray-50 cursor-not-allowed text-gray-600">
                        <option selected>Земельные участки</option>
                    </select>
    ...
    @auth
        @if(Auth::user()->isAdmin() || Auth::user()->isAgent())
            <div class="mb-6 flex justify-end">
                <a href="{{ route('properties.create', ['type' => 'plot']) }}"
                ...
                <div class="group bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 will-change-transform">
                    <div class="relative h-56 overflow-hidden rounded-t-2xl">
                        @php
                            ...
                            $firstImage = !empty($images) && is_array($images) && isset($images[0])
                        ...
                        <div class="absolute bottom-3 left-3 bg-black/50 backdrop-blur-sm text-white px-3 py-1 rounded-lg text-xs z-10">
                            <i class="fas fa-tree mr-1"></i> Участок
                        </div>
                    </div>
    ...
    @else
        <div class="text-center py-16 bg-white rounded-2xl shadow-lg">
            ...
            <h3 class="text-2xl font-semibold text-gray-700 mb-2">Участки не найдены</h3>
        ...
        <div>
            ...
            <div class="mb-6">
                <div class="relative rounded-xl overflow-hidden">
                    <img id="mainImage" src="${imageUrls[0]}" alt="${escapeHtml(property.title)}" class="w-full h-96 object-cover">
                    ${imageUrls.length > 1 ? `
                ...
                <div class="lg:col-span-2">
                    ...
                    <div class="grid grid-cols-2 gap-3 mb-6">
                        ...
                        <div class="flex items-center text-gray-700 bg-gray-50 p-3 rounded-xl">
                            ...
                            <span class="ml-2 text-sm">${property.area} сот.</span>
                        </div>
                        ${property.rooms ? `
                        <div class="flex items-center text-gray-700 bg-gray-50 p-3 rounded-xl">
                ...
                <div class="lg:col-span-1">
                    <div class="bg-gradient-to-br from-gray-50 to-white rounded-xl p-6 sticky top-4 border border-gray-100 shadow-lg">
                        ...
                        <div class="mt-4 pt-4 border-t border-gray-200">
                            <button onclick="window.location.href='tel:${agentPhone}'" class="w-full gradient-bg text-white py-2.5 rounded-xl hover:shadow-lg transition-all duration-300 font-medium mb-2 flex items-center justify-center gap-2">
                                <i class="fas fa-phone"></i> Позвонить
                            </button>
                            ${agentWhatsapp ? `
                            <button onclick="window.open('https://wa.me/${agentWhatsapp.replace(/[^0-9]/g, '')}', '_blank')" class="w-full bg-green-500 text-white py-2.5 rounded-xl hover:bg-green-600 transition-all duration-300 font-medium flex items-center justify-center gap-2">
