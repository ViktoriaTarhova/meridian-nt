{{-- resources/views/properties/houses.blade.php --}}
@extends('layouts.app')
...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="relative rounded-2xl overflow-hidden mb-12 bg-gradient-to-r from-purple-600 to-indigo-600">
        ...
        <div class="relative z-10 py-10 px-8 text-white">
            <h1 class="text-4xl md:text-5xl font-bold mb-3">Дома</h1>
            <p class="text-lg md:text-xl opacity-90">Найдите дом своей мечты в лучших районах города</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-xl p-6 mb-8 border border-gray-100">
        <form method="GET" action="{{ route('properties.houses') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            <div>
    ...
    @auth
        @if(Auth::user()->isAdmin() || Auth::user()->isAgent())
            <div class="mb-6 flex justify-end">
                <a href="{{ route('properties.create', ['type' => 'house']) }}"
                    ...
                    <div class="relative h-56 overflow-hidden rounded-t-2xl">
                        ...
                        @auth
                            <button onclick="toggleFavorite({{ $property->id }}, this)"
                                ...
                                <i class="fas fa-heart {{ in_array($property->id, $favoriteIds) ? 'text-red-500' : 'text-gray-400' }} text-xl"></i>
                            </button>
                        @endauth

                        {{-- Бейдж с типом --}}
                        <div class="absolute bottom-3 left-3 bg-black/50 backdrop-blur-sm text-white px-3 py-1 rounded-lg text-xs z-10">
                            <i class="fas fa-home mr-1"></i> Дом
                        </div>
                    </div>
    ...
    @else
        <div class="text-center py-16 bg-white rounded-2xl shadow-lg">
            ...
            <h3 class="text-2xl font-semibold text-gray-700 mb-2">Дома не найдены</h3>
